...
Random.Unit <-function(n, dim, threshold) {	
  #definisi fungsi Random.Unit() dengan parameter n (banyaknya data), dim (dimensi), dan threshold
  ...
  # mengembalikan sebuah matriks yang berupa gabungan points dan label satu matriks
}

Classify <- function(x, weights) {	
  #definisi fungsi Classify() dengan parameter x (data observasi) dan w (bobotnya).
  return(sign(x %*% weights))		
  #mengembalikan nilai(tanda/sign) -1, 0, atau 1 berdasarkan hasil dot product antara x dan w 
}
  ...
  while (misclassfied) {	# loop hingga semua data tidak misclassified
    ...
    for (i in 1:n) {		  # Untuk setiap data observasi...
      if (label[i] * Classify(obs[i , ], w) <= 0) {	
        # ...jika label hasil perceptron (rule klasifikasi) tidak sama dengan label yang sudah ditetapan di awal...
...
Plot3D <- function(points, a, b, c, d) {		
  # definisi fungsi PLot3D() dengan parameter points (data input), a, b, c, d
